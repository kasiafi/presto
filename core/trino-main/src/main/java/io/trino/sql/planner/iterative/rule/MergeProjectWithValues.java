        ...
        implements Rule<ProjectNode>
{
    ...
    public Result apply(ProjectNode node, Captures captures, Context context)
    {
        ...
        Map<Symbol, Expression> assignments = ImmutableMap.copyOf(node.getAssignments().getMap());
        ...
        if (valuesNode.getOutputSymbols().isEmpty()) {
            return Result.ofPlanNode(new ValuesNode(
                    ...
                    nCopies(valuesNode.getRowCount(), new Row(ImmutableList.copyOf(assignments.values())))));
        }
        ...
        Set<Symbol> multipleReferencedSymbols = node.getAssignments().getExpressions().stream()
        ...
        for (Expression rowExpression : valuesNode.getRows().get()) {
            ...
            Row projectedRow = new Row(assignments.values().stream()
                    .map(expression -> replaceExpression(expression, mapping))
                    .collect(toImmutableList()));
            projectedRows.add(projectedRow);
        }
        return Result.ofPlanNode(new ValuesNode(valuesNode.getId(), node.getOutputSymbols(), projectedRows.build()));
    }
