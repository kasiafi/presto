...
The ``MATCH_RECOGNIZE`` clause is used to detect patterns across rows of a
table. Patterns of interest are specified using row pattern syntax based on
regular expressions. When a match to the pattern is detected, one or more rows
are returned. They contain requested information about the match.
...
ordered data set. For example, the statement ``ORDER BY orderdate`` allows for
matching on a trend of changes over time, since it matches a sequence of rows::
...
``LAST(D.totalprice) AS top_price`` returns the highest price in the match. It
is the price in the last row associated with ``D``, which is also the final row
of the match.

Measure expressions can refer to the columns of the input table. They also
allow special syntax (see :ref:`pattern_recognition_expressions`).
Each row pattern measure defines an output column of the pattern recognition.
The column can be referenced with the ``measure_name``. The ``MEASURES`` clause
is optional. When no row pattern measures are specified, certain input columns
(depending on :ref:`ROWS PER MATCH<rows_per_match>` clause) are the output of
...
For ``ALL ROWS PER MATCH``, every row of a match produces an output row. Output
consists of ``PARTITION BY`` columns, ``ORDER BY`` columns, measures and
remaining columns from the input table. By default, empty matches are shown and
unmatched rows are skipped, similarly as with ``ONE ROW PER MATCH`` option.
However, this behavior can be changed by modifiers::
...
It is a sequence of components without operators between them:
...
The order of alternatives defines the order of preference:
...
The order of preference is the lexicographical order established by the order
of components. In the above example, the most preferred option is ``A B C``,
and the least preferred option is ``C B A``.
...
variables ``B`` and ``C``. Union variables are useful in ``MEASURES``,
...
to pattern variable ``A``, and then searches backward until it finds two more
occurrences of variable ``A`` within the match. The result is the value of the
``totalprice`` column in that row.

With the ``first`` and ``last`` functions the result is ``null``, if the
searched row is not found in the mach. The second argument is optional. The
default value is ``0``, which means that by default these functions navigate to
the first or last row of interest. If specified, the second argument must be a
...
bounds, the result is ``null``. The second argument is optional. The default
value is ``1``, which means that by default these functions navigate to
previous or next row. If specified, the second argument must be a non-negative
integer number.
